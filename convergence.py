...
def plot_errors(srcfile):
    ...
    # TODO: @ For some reason, covariance is equal across all timescales.
    ...
    errors = dict(complex=dict(), mag=dict(), phase=dict(), covariance=dict())
    ...
    for i, runspec in enumerate(runs):
        ...
        analytic_covariance = run['covariances']['analytic']

        for covtype, covariance in run['covariances'].iteritems():
            if covtype != 'analytic':
                if covtype not in errors['covariance']:
                    errors['covariance'][covtype] = np.zeros(
                        (len(runs), nvars, nvars)
                    )

                errors['covariance'][covtype][i] = abs(
                    covariance - analytic_covariance
                )**2
    ...
    xlabels = dict(
        ...
        covariance='$\\log |\\hat{\\Sigma}_{xy} - \\Sigma_{xy}|^2$'
    )

    print pbdict.dictpaths(errors).keys()
...
def run_config(srcfile, forceseries=False, forcespectra=False):
    ...
    for config, cname in izip(configs.combos, configs.hyphenate_changes()):
        ...
        noise_covariance = np.array(pbdict.getkeypath(
        ...
        if forcespectra or not os.path.exists(outfile):
            ...
            covariances = dict(
                linear=np.cov(linear),
                nonlinear=np.cov(nonlinear),
                analytic=model.calculate_covariance(
                    sym_params, noise_covariance
                )
            )
            ...
            spectra['analytic'] = np.array([
                model.calculate_spectrum(sym_params, noise_covariance, v)
                for v in freqs
            ]).T
